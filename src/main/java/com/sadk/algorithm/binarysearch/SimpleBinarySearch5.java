...
public class SimpleBinarySearch5 implements BinarySearch<Integer> {
    ...
    public int search(Integer[] values, Integer target) {
        ...
        if (0 == values.length) {
            ...
            //(488,1000] lower=488=>
            //  values[lower+i] < target=> lower=lower+i => still: [values[lower]<target]<=values[lower+i] (744,1000]
            //  values[lower+i] >=target=> still: values[lower]<[target<=values[lower+i]] (488,744]
            //(-1,511] lower=-1=>
            //  values[lower+i] < target=> lower=lower+i => still: [values[lower]<target]<=values[lower+i] (255,511]
            //  values[lower+i] >=target=> still: values[lower]<[target<=values[lower+i]] (-1,255]
            if (values[lower + 128] < target) {
                lower += 128;
            }
            //assert: i=2^7(128) && values[lower]<target<=values[lower+i]
            //(255,511] lower=255=>
            //  values[lower+i] < target=> lower=lower+i => still: [values[lower]<target]<=values[lower+i] (383,511]
            //  values[lower+i] >=target=> still: values[lower]<[target<=values[lower+i]] (255,383]
            //(-1,255] lower=-1=>
            //  values[lower+i] < target=> lower=lower+i => still: [values[lower]<target]<=values[lower+i] (127,255]
            //  values[lower+i] >=target=> still: values[lower]<[target<=values[lower+i]] (-1,127]
